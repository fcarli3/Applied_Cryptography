...
int main() {
    ...
    /* For this exercise I assume to use the first 16 bytes of the secret (bad practice!) */
    unsigned char* shared_secret_key = (unsigned char*)malloc(iv_len);
    
    /* Take the first 16 bytes from the secret to create the shared secret key */
    memcpy(shared_secret_key, secret, iv_len);
